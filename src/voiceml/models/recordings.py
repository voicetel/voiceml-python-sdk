...
class RecordingList(_Base):
    ...
    The account-scoped endpoint (``GET /Recordings``) returns the full
    Twilio-compatible pagination fields (``recordings/page/page_size/total``).
    Per-call (``GET /Calls/{sid}/Recordings``) and per-conference
    (``GET /Conferences/{sid}/Recordings``) endpoints currently return only
